...
from _judge import judge_sharded, run_haiku
...
CODE_STYLE_PROMPT = """You are a strict code reviewer. Below is content the assistant just wrote into a file, the text that content REPLACED, the current COMMITTED version of the file, and code-style feedback rules from the user.
...
BASH_PROMPT = """You are a strict code reviewer. Below is a bash command the assistant just ran, and code-style feedback rules from the user.
...
TEST_WEAKENING_PROMPT = """You are reviewing one Edit to a test file. Below is the exact text the edit REPLACED (PREVIOUS) and the text it WROTE (NEW).
...
def judge_code_style(
    ...
    return judge_sharded(
        memories,
        lambda rules: CODE_STYLE_PROMPT.format(
            file_path=file_path,
            replaced=replaced,
            committed=committed,
...
def judge_bash_command(command: str, memories: list[tuple[str, str]]) -> list[str]:
    return judge_sharded(
        memories,
        lambda rules: BASH_PROMPT.format(
            command=command[:MAX_CONTENT_CHARS], rules=rules
        ),
        timeout=TIMEOUT_S,
    )
...
def judge_test_weakening(previous: str, content: str) -> bool:
    prompt = TEST_WEAKENING_PROMPT.format(
        previous=previous[:MAX_CONTENT_CHARS], content=content[:MAX_CONTENT_CHARS]
    )
    verdict = re.sub(r"[*`_]", "", run_haiku(prompt, timeout=TIMEOUT_S))
